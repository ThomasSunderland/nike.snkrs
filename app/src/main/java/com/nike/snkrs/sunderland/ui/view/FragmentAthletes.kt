            ...
            with (binding.foregroundViewPager) {
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.athletes.collectLatest { athleteData ->
                            if (viewModel.firstTimeThrough.value == true && athleteData.isNotEmpty()) {
                                // transform the received data to a representation understood by our adapter
                                (adapter as AdapterGeneral).addAll(athleteData.map { AdapterGeneralItem(it.uid, it.resource, it.source) })
