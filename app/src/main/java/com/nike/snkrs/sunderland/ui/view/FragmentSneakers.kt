...
import com.nike.snkrs.sunderland.ui.viewmodel.ViewModelSneakers
...
/**
 * Used to display a carousel of sneaker images
 * @author Thomas Sunderland. 2021 MAY 09
 */
class FragmentSneakers : Fragment(), CoroutineScope {
     ...
     */
    private lateinit var viewModel: ViewModelSneakers
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return FragmentSneakersBinding.inflate(inflater, container, false).apply {
            tryCatch {
                this@FragmentSneakers.binding = this
                this@FragmentSneakers.viewModel = ViewModelProvider(requireActivity()).get(ViewModelSneakers::class.java)

                viewModel = this@FragmentSneakers.viewModel
                lifecycleOwner = this@FragmentSneakers
            }
        }.root
    }
    //@formatter:on
            ...
            with (binding.remoteData) {
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.remoteSneakersData.collectLatest { sneakersData ->
                            (adapter as AdapterRemoteData).submitList(sneakersData.map { AdapterRemoteDataItem(it.uid, it.resource, it.source) })
                        }
                    }
                }
            }
    ...
    private fun configureViewPager() {
        ...
        tryCatchWithLogging({
            with (binding.localData) {
                ...
                // update view model as page changes so that if we leave and come back to this fragment the last page can be restored
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                        viewModel.localSneakersData.collectLatest { sneakersData ->
                            (adapter as AdapterLocalData).submitList(sneakersData.map { AdapterLocalDataItem(it.uid, it.resource, it.source) })
                        }
                    }
                }
            }
